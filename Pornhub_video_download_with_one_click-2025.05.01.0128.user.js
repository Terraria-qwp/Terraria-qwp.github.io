...
// @downloadURL https://update.sleazyfork.org/scripts/528800/Pornhub%20video%20download%20with%20one%20click.user.js
// @updateURL https://update.sleazyfork.org/scripts/528800/Pornhub%20video%20download%20with%20one%20click.meta.js
// ==/UserScript==
...
GM_addStyle(`
    ...
    .download-url-copy { background: #4299e1; } /* 蓝色：复制 */
    .download-url-mp4 { background: #22c55e; } /* 绿色：下载 */
...
// 替换 VideoParsing.injectUrls2Dom 方法，保持 DOM 结构但适配新样式
static injectUrls2Dom(urlInfo) {
    ...
    urlInfo.forEach(item => {
        ...
        <li>
            ...
            <input class="download-url-input" value="${item.url}" readonly /> <!-- 加readonly避免误编辑 -->
            <a class="download-url-copy" data-href="${item.url}" href="javascript: void(0);">${translate('copydownloadbtn')}</a>
            <a class="download-url-mp4" data-href="${item.url}" href="javascript: void(0);">${translate('downloadbtn')}</a>
        </li>
        `)
    })
    // pc和非PC两种情况都加上，避免重复显示（加判断去重）
